<?php
    ...
    if(isset($_SESSION['id'])){
        $display_members = $db->prepare("SELECT * FROM user WHERE id <> ?");
        $display_members->execute(array($_SESSION['id']));
    }else {
...
<body class="bg-[#1D1D1D]">
    ...
    <div class="flex justify-center">
        <div class="flex flex-col items-center bg-[#343434] m-20  py-10  px-8 rounded">
                        ...
                        <?php echo $am['first_name'] . ' ' . $am['last_name']?>
                            ...
                            <div class="flex flex-col mt-4 space-y-3 lg:mt-6">
                                <form action="updateUser.php" method="post">
                                    ...
                                    <button type="submit" class="mt-6 text-white px-4 py-2 text-center rounded-md text-1xl font-medium bg-[#1D1D1D] hover:bg-black transition duration-300">Modifier utilisateur</button>
                                </form>
                                <form action="../controller/deleteUser.php" method="post">
                                    <input type="hidden" id="id" name="id" value="<?php echo $am['id'] ?>">
                                    <button type="submit" class="mt-3 text-white px-4 py-2 text-center rounded-md text-1xl font-medium bg-[#F14747] hover:bg-[#d94040] transition duration-300">Supprimer utilisateur</button>
                                </form>
                            </div>
                        </div>
                    </div>
            ...
            <a href="addUser.php" class="border-2 mt-10 border-[#F14747] text-white px-4 py-2 rounded-md text-1xl font-medium hover:bg-[#F14747] transition duration-300 ">Ajouter un utilisateur</a>
